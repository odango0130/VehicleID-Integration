...
def main():
    ...
    # 距離の閾値（ユーザーが指定）
    distanceThreshold = 500  # 例として50ピクセルを設定
    discardFrames = 100  # 例として10フレームを設定
...
def predict_lost_bbox(lost_bboxes, frame_number):
    for lost_id, bboxes in lost_bboxes.items():
        if len(bboxes) > 1:
            # 速度の計算
            dx = bboxes[-1]['bbox'][0] - bboxes[-2]['bbox'][0]
            dy = bboxes[-1]['bbox'][1] - bboxes[-2]['bbox'][1]
            predicted_bbox = [bboxes[-1]['bbox'][0] + dx, bboxes[-1]['bbox'][1] + dy, bboxes[-1]['bbox'][2] + dx, bboxes[-1]['bbox'][3] + dy]
