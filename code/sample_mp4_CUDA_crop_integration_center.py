...
def main():
    ...
    while True:
       ...
       #----------------------------------------------------#
        ...
        if scale_factor < 1:            # 現在ののbboxが初期より小さくなったとき
            ...
            precenter_x -= (cap_width*2 - scaled_width)
            precenter_x -= (cap_height*2 - scaled_height)
            ...
            scaled_image =  cv2.copyMakeBorder(
                ...
                cv2.BORDER_CONSTANT, value=[0, 0, 0])
            precenter_x += extend_left
            precenter_y += extend_top
        elif scale_factor == 1:
    ...
    create_rolling_median_video(frames_array, 12, cap_fps, precenters_x, precenters_y)
    cap.release()
    writer.release()

def create_rolling_median_video(frames, per_median, fps, precenters_x, precenters_y):
    frame_count, frame_height, frame_width, _ = frames.shape
    # ビデオライターを初期化
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter('rolling_median_video.mp4', fourcc, fps, (frame_width/2, frame_height/))
    ...
    for i in range(0, frame_count - frame_per_median):
        subset_x = precenters_x[i:i+frame_per_median]
        subset_y = precenters_y[i:i+frame_per_median]
        average_center_x = int(sum(subset_x)/len(subset_x))
        average_center_y = int(sum(subset_y)/len(subset_y))
        median_frame = compute_median_for_frames(frames, i, i + frame_per_median)
        median_frame_center = cv2.getRectSubPix(
            median_frame, (frame_height/2,frame_width/2), (average_center_x, average_center_y)
            )
        out.write(median_frame_center)
