...
def main():
    ...
    while True:
        ...
        debug_image = draw_debug(debug_image, active_bboxes)
        # 書き込み
        ...
        #---------------------------------------------------------#
        if frame_number == 13:
            cv2.imwrite('13f_least.png', debug_image)
        if frame_number == 50:
            cv2.imwrite('50f_least.png', debug_image)
    cap.release()
    writer.release()
...
def draw_debug(image, lost_bboxes):
    ...
    for object_id, bbox_info in lost_bboxes.items():
        ...
        # 左上の座標を描画
        cv2.circle(image, top_left, 20, (0, 0, 255), -1)

        # IDを描画
        cv2.putText(image, str(object_id), (top_left[0] + 10, top_left[1] + 10), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 255), 2)
    return image
...
def draw_debug2(image, bboxes):
    ...
    for object_id, bbox_info in bboxes.items():
        ...
        if object_id == '2_1':
            # バウンディングボックスを描画
            cv2.rectangle(image, top_left, bottom_right, (0, 0, 255), 14)

            # IDを描画
            cv2.putText(image, str(object_id), (top_left[0], top_left[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 4)
        else:
            # バウンディングボックスを描画
            cv2.rectangle(image, top_left, bottom_right, (0, 255, 0), 14)
    
            # IDを描画
            cv2.putText(image, str(object_id), (top_left[0], top_left[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 4)
