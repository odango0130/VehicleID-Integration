...
def main():
    ...
    # 出力ファイル
    if args.movie is not None:
        outpath = os.path.splitext(args.movie)[0] + "_predict.mp4"
    else:
        outpath = "camera.mp4"
    ...
    while True:
        ...
        predict_lost_bbox(lost_bboxes, frame_number)
...
def predict_lost_bbox(lost_bboxes, frame_number):
    for lost_id, bboxes in lost_bboxes.items():
        if len(bboxes) > 1:
            # 速度の計算
            dx = bboxes[-1]['bbox'][2] - bboxes[-2]['bbox'][2]  # x方向の変化量
            dy = bboxes[-1]['bbox'][3] - bboxes[-2]['bbox'][3]  # y方向の変化量
            # 予測されるバウンディングボックスの計算
            predicted_bbox = [bboxes[-1]['bbox'][0] + dx, bboxes[-1]['bbox'][1] + dy,
                              bboxes[-1]['bbox'][2] + dx, bboxes[-1]['bbox'][3] + dy]
    ...
    for t_id, bbox in zip(t_ids, t_bboxes):
        ...
        if t_id not in active_bboxes:       #新しいIDか
            ...
            for lost_id, bboxes in lost_bboxes.items():     #lostID全てをforで回す
                distance = np.linalg.norm(np.array([bbox[2], bbox[1]]) - np.array([bboxes[-1]['bbox'][2], bboxes[-1]['bbox'][1]]))
...
def draw_debug(image, lost_bboxes):
    ...
    for object_id, bbox_info in lost_bboxes.items():
        ...
        cv2.putText(image, str(object_id), (top_right[0] + 10, top_right[1] + 10), ...)
    return image
...
def draw_debug2(image, bboxes):
    ...
    for object_id, bbox_info in bboxes.items():
        ...
        top_right = (int(bbox[2]), int(bbox[1]))  # 追加：右上の座標

        # バウンディングボックスを描画
        cv2.rectangle(image, top_left, bottom_right, (0, 255, 0), 2)

        # IDを右上の座標に基づいて描画
        cv2.putText(image, str(object_id), (top_right[0] - 50, top_right[1] + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
