...
def main():
    ...
    # 出力ファイル
    if args.movie is not None:
        outpath = os.path.splitext(args.movie)[0] + "_car_center_.mp4"
    else:
        outpath = "camera.mp4"
    ...
    while True:
       ...
       #-------------------車中心画像作成---------------------------------#
        ...
        # bordered_imageの中心座標
        bimage_center_x = cap_width
        bimage_center_y = cap_height
        
        if scale_factor < 1:            # 現在ののbboxが初期より小さくなったとき
            scaled_image = cv2.getRectSubPix(           # 外側の画素を削って全体に対するbboxの割合を初期と同じに
                bordered_image, (scaled_width, scaled_height), (bimage_center_x, bimage_center_y)
            )
    ...
    """create_center_adjust_video(
        frames_array, cap_fps, scale_factors, precenters_x, precenters_y,
        cap_width, cap_height,
        desired_width, desired_height)"""
    create_center_adjust_rolling_video(
        frames_array, cap_fps, scale_factors, precenters_x, precenters_y,
        cap_width, cap_height,
        desired_width, desired_height,
        12)
...
def create_center_adjust_rolling_video(
    ...
    # 拡大縮小実行for文
    for index, (scale_factor, center_x, center_y) in enumerate(zip(inverse_scaled_factors, precenters_x, precenters_y)[skip_start:-skip_end]):
        ...
        if scale_factor < 1:            # 現在ののbboxが初期より小さくなったとき
            scaled_image = cv2.getRectSubPix(           # 外側の画素を削って全体に対するbboxの割合を初期と同じに
                median_frames, (scaled_width, scaled_height), (mimage_center_x, mimage_center_y)
            )
        elif scale_factor > 1:          # 今のbboxが初期より大きくなったとき
            extend_width = scaled_width - cap_width*2
            extend_height = scaled_height - cap_height*2
            ...
            extend_right = int(extend_width*0.5)
            scaled_image =  cv2.copyMakeBorder(
                median_frames, extend_top, extend_bottom, extend_left, extend_right,
                cv2.BORDER_CONSTANT, value=[0, 0, 0])
        elif scale_factor == 1:
            scaled_image = median_frames
        # フレームのリサイズ（指定された解像度に変更）
        resized_frame = cv2.resize(scaled_image, (desired_width, desired_height), interpolation=cv2.INTER_LINEAR)
